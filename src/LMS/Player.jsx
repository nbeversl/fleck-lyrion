...
class Player {
    ...
    constructor(LMS, address) {
        ...
        this.playAlbum = (albumTitle) => {
            this.LMS.request([this.address,["playlist", "loadalbum", '*', '*', albumTitle]], (r) => {
                this.play();
            });
        }
        ...
        this.playTrack = (id) => { 
            this.LMS.request([this.address,["playlist","clear"]],(r) => {
                this.LMS.request([this.address,["playlistcontrol", "cmd:add", "track_id:"+id.toString()]],(r) => {
                    this.LMS.request([this.address,["play"]]);
                });
            });
        }
        ...
        this.addTrack= (id) => {
            this.LMS.request([this.address,["playlistcontrol", "cmd:add", "track_id:"+id.toString()]]);
        }

        this.savePlayList = (filename) => {
            this.LMS.request([this.address,["playlistcontrol", "cmd:save", filename]]);
        }

        this.setVolume = (value) => {            
            this.LMS.request([this.address,["mixer", "volume", value.toString()]],(r) => { 
                this.volume = value;
            });
        }
        ...
        this.playAlbumFromTrackAndContinue = (track, startNumber) => { 
            ...
            this.LMS.request([this.address,["playlist","clear"]],(r) => {                
                this.LMS.request([this.address,["playlist", "addtracks", "album.id="+albumID]], (r) => {
                    this.LMS.request([this.address,["playlist", "index",'+'+startNumber.toString()]]);
                });
            });
        ...
        this.getPlayerStatus = (callback) => {
            this.LMS.request([this.address,[ 
                "status","0","100",
                "tags:duration,time, mode, **playlist index**, **l**,**J**,**K**"]], (r) => {                     
                    callback(r.result);
                });
        }
